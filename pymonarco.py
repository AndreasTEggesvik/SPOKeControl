...
#
####### Counter constants #######
MONARCO_SDC_REG_CNT1CFG = 0x024 # COUNTER1 Configuration [W], see MONARCO_SDC_COUNTER_MODE_* 
MONARCO_SDC_REG_CNT2CFG = 0x025 # COUNTER2 Configuration [W], see MONARCO_SDC_COUNTER_MODE_* 
#
...
#MONARCO_SDC_COUNTER_CTRL_EXT = (1 << MONARCO_SDC_COUNTER_CTRL__SHIFT) # /* direction selectable by B input, high = down-counting */
...
class Monarco(threading.Thread):
    def __init__(self, lib_path, spi_interface='/dev/spidev0.0', spi_clock=4000000,
        ...
        self.__cnt1_mode = _monarco_sdc_item_t()
        self.__cnt2_mode = _monarco_sdc_item_t()
        self.__sdc_counter = 0
...
################ New code for counting ##################

    def initiate_counters_quad(self):
        global MONARCO_SDC_REG_CNT1CFG, MONARCO_SDC_COUNTER_MODE_QUAD, MONARCO_SDC_REG_CNT2CFG
        with self.__mutex:
            self.__cxt.sdc_items[self.__cxt.sdc_size].address = ctypes.c_uint16(MONARCO_SDC_REG_CNT1CFG)
            self.__cxt.sdc_items[self.__cxt.sdc_size].value = ctypes.c_uint16(MONARCO_SDC_COUNTER_MODE_QUAD)
            self.__cxt.sdc_items[self.__cxt.sdc_size].write = ctypes.c_uint8(1)
            self.__cxt.sdc_items[self.__cxt.sdc_size].request = ctypes.c_uint8(1)
        self.__cxt.sdc_size += 1

        with self.__mutex:
            self.__cxt.sdc_items[self.__cxt.sdc_size].address = ctypes.c_uint16(MONARCO_SDC_REG_CNT2CFG)
            self.__cxt.sdc_items[self.__cxt.sdc_size].value = ctypes.c_uint16(MONARCO_SDC_COUNTER_MODE_QUAD)
            self.__cxt.sdc_items[self.__cxt.sdc_size].write = ctypes.c_uint8(1)
            self.__cxt.sdc_items[self.__cxt.sdc_size].request = ctypes.c_uint8(1)
        self.__cxt.sdc_size += 1
...
# Counter_identifier = [1,2], mode = [OFF, QUAD, PULSE_COUNT], edge_count = [RISE, FALL, BOTH]
    def initiate_counter(self, counter_identifier, mode, edge_count):
        sdc_item = self.__sdc_counter
        
        if mode == 'OFF':
                mode = False
        elif mode == 'QUAD':
                mode = self.MONARCO_SDC_COUNTER_MODE_QUAD 
        elif mode == 'PULSE_COUNT':
                mode = self.MONARCO_SDC_COUNTER_MODE_PCNT
                if edge_count == 'RISE':
                        mode |= self.MONARCO_SDC_COUNTER_EDGE_RISE
                elif edge_count == 'FALL':
                        mode |= self.MONARCO_SDC_COUNTER_EDGE_FALL
                elif edge_count == 'BOTH':
                        mode |= self.MONARCO_SDC_COUNTER_EDGE_BOTH
                        
                        
        if counter_identifier == 1:
                self.__cnt1_mode = self.__cxt.sdc_items[sdc_item]
                self.__cxt.sdc_items[sdc_item] = _monarco_sdc_item_t(address = self.MONARCO_SDC_REG_CNT1CFG, 
                                                                        value= mode, 
                                                                        write = 1, 
                                                                        request = 1)
                self.__cxt.sdc_size += 1 #This line makes things happen, and gives the following text: 
                                                              # [VERB] monarco_sdc_tx: No SDC request in this cycle
                
        elif counter_identifier == 2: 
                self.__cnt2_mode = self.__cxt.sdc_items[sdc_item]
                self.__cxt.sdc_items[sdc_item] = _monarco_sdc_item_t(address = self.MONARCO_SDC_REG_CNT2CFG, 
                                                                        value= mode, 
                                                                        write = 1, 
                                                                        request = 1)
                self.__cxt.sdc_size += 1 #This line makes things happen, and gives the following text: 
        ...
        return self.__sdc_counter
                                                              
        
#############################################

    def read_counter(self, counter_identifier):
#        self.__cxt.sdc_size += 1 #This line makes things happen, and gives the following text: 
                                # [VERB] monarco_sdc_tx: No SDC request in this cycle
        if counter_identifier == 1:
            with self.__mutex:
                a = self.__cxt.rx_data.cnt1
              
        elif counter_identifier == 2:
            with self.__mutex:
                a = self.__cxt.rx_data.cnt2
        self.__cxt.sdc_size = 0
        return a
